classdef CurveBase < handle
  ...
  methods
    ...
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    function [ p1, p2 ] = points( self, numpoints )
      % Alessandro Antonucci @AlexRookie
      % University of Trento
      np = linspace(0, ClothoidCurveMexWrapper( 'length', self.objectHandle ), numpoints);
      p1 = NaN(1,numpoints);
      p2 = NaN(1,numpoints);
      for i = 1:numpoints
          [ X, Y ] = ClothoidCurveMexWrapper( 'eval', self.objectHandle, np(i) );
          p1(i) = X;
          p2(i) = Y;
      end
